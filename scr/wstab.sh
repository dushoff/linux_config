...
## Already here.
i3-msg "[$crit workspace=\"__focused__\"] focus" 2>/dev/null | grep -q '"success":true' && exit 0
...
if [ -n "$title" ] \
	&& i3-msg "[$crit] move to workspace current, focus" 2>/dev/null | grep -q '"success":true'
then
	exit 0
fi
...
for i in $(seq 1 50); do
	i3-msg "[$wcrit] move to workspace current, focus" 2>/dev/null | grep -q '"success":true' && exit 0
	sleep 0.1
done
